...
RSpec.describe Rental do
  ...
  describe '#duration_fee' do
    ...
    context 'with no discount' do
      let(:rental) { Rental.new(car, '2018-4-14', '2018-4-14', 10) }
      it { is_expected.to eq(1*100) }
    end
    context 'with a discount' do
      it { is_expected.to eq(2*100 - 1*100*0.1) }
    end
  end

  describe '#discount_for_day' do
    subject { rental.send(:discount_for_day, day) }
    
    context 'when it is the first day' do
      let(:day) { 0 }
      it { is_expected.to eq(1) }
    end
    
    context 'when it is the second day' do
      let(:day) { 2 }
      it { is_expected.to eq(0.9) }
    end
    
    context 'when it is the fourth day' do
      let(:day) { 4 }
      it { is_expected.to eq(0.7) }
    end
    
    context 'when it is the tenth day' do
      let(:day) { 10 }
