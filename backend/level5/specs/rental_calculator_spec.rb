...
RSpec.describe RentalCalculator do
  ...
  let(:options_hash) { [{ "id" => 1, "rental_id" => 1, "type" => "gps" },
                        { "id" => 2, "rental_id" => 1, "type" => "baby_seat" },
                        { "id" => 3, "rental_id" => 2, "type" => "additional_insurance" }] }

  subject { RentalCalculator.new(cars_hash, rentals_hash, options_hash) }
  ...
  describe '#get_rental_prices' do
    ...
    it "has id, price and commission keys for each object" do
      expect(subject().first.keys).to eq([:id, :options, :actions])
    end
  end

  describe '#option_types_by_rental' do
    subject { super().send :option_types_by_rental }

    it { is_expected.to be_a(Hash) }
    its(:size) { is_expected.to eq(2) }
    it 'has the good values' do
      expect(subject[1]).to eq(['gps', 'baby_seat'])
    end
  end
end